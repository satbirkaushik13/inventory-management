...
const update = (req, res, next) => {
    ...
    db.query(sql, [...values, item_id], (err, result) => {
        ...
        if (result.affectedRows === 0) {
            res.status(404);
            return next(new Error("Item not found or no changes made"));
        }
...
const getKeywords = (req, res, next) => {
    ...
    if (!item_id) {
        res.status(400);
        return next(new Error("No item ID was provided."));
    }
    ...
    db.query(sql, [item_id], (err, results) => {
        ...
        if (results.length === 0) {
            res.status(404);
            return next(new Error("No keyword found for the given item."));
        }
...
const bindKeywords = (req, res, next) => {
    let { item_id } = req.params;
    ...
    if (!Array.isArray(keywords) || keywords.length === 0) {
        res.status(400);
        return next(new Error("Invalid or empty keywords array."));
    }
    ...
    if (keywordIds.length === 0) {
        res.status(400);
        return next(new Error("No valid keyword IDs provided."));
    }
    ...
    db.query(itemKeywordInsertQuery, [itemKeywordValues], (err, linkResults) => {
        if (err) {
            ...
            res.status(500);
            return next(new Error("Database error while linking keywords."));
        }
...
const uploadAttachment = (req, res, next) => {
    ...
    if (!folder) {
        res.status(400);
        return next(new Error("Invalid attachment_type"));
    }
    ...
    db.query(sql, [item_id, req.file.filename, path.join("uploads", "images"), attachment_type], (err, result) => {
        if (err) {
            console.error("❌ Database error:", err);
            res.status(500);
            return next(new Error("Database error while saving attachment"));
        }
